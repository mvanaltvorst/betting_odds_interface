...
def calculate_probabilities():
    ...
    methods = [
        ("Implied Ask", betting_implied_ask(match_odds)),
        ("Implied Mean", betting_implied_mean(match_odds)),
        ("Implied Advancing Mean", advancing_probs(state['team1_advancing_odds'], state['team2_advancing_odds'])),
    ]
    data = []
    for method, probs in methods:
        row = [method] + [f"{prob*100:.1f}%" for prob in probs]
        data.append(row)
    df = pd.DataFrame(data, columns=["Method", state['team1_name'], "Draw", state['team2_name']])
    return df.to_dict('records')

def calculate_implied_margin():
    match_odds = [state['team1_odds'], state['draw_odds'], state['team2_odds']]
    implied_margin = np.sum(1 / np.array(match_odds)) - 1
    return f"{implied_margin:.2%}"
...
def handle_update_odds(data):
    ...
    implied_margin = calculate_implied_margin()
    emit('odds_updated', {'state': state, 'probabilities': probabilities, 'implied_margin': implied_margin}, broadcast=True)
